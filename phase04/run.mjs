#!/usr/bin/env node
/**
 * Phase 04 — FINAL EVIDENCE CLOSURE runner
 * CV-01..CV-44 pure computational conformance harness.
 *
 * Rules (strict):
 * - Expected values ONLY from fixtures/CV-*.json (never from implementation)
 * - Default mode: VERIFY ONLY — does NOT write phase04-report.json → clean worktree
 * - Optional --write: produces stable report (provenance only, no mutable git SHA head)
 * - Real JSON Schema validation via schema-validate.mjs
 * - expectedStatus vs actualStatus check on every CV
 * - Determinism: double-run equality
 * - No network, DB, providers, randomness, wall-clock side effects
 */
...
/** Derive actualStatus independently from the computation result. */
function deriveActualStatus(actual) {
  if (!actual || typeof actual !== "object") return "FAIL";
  if ("status" in actual) {
    const s = actual.status;
    if (s === "PASS" || s === "conformant" || s === "ok") return "PASS";
    if (
      s === "REJECTED" ||
      s === "non-conformant" ||
      s === "rejected_snapshot" ||
      s === "graph_integrity_violation"
    )
      return "REJECTED";
    return String(s);
  }
  // pure value outputs that matched expectedOutput are PASS
  return "PASS";
}
...
function main() {
  let runtimeHead = "unknown";
  try {
    runtimeHead = execSync("git rev-parse HEAD", { cwd: ROOT }).toString().trim();
  } catch {}
  ...
  for (let n = 1; n <= 44; n++) {
    ...
    const expectedStatus = fx.expectedStatus || "PASS";
    ...
    const actualStatus = deriveActualStatus(r1.actual);
    const statusMatch = actualStatus === expectedStatus;
    if (!statusMatch) statusMatchFail = true;

    if (r1.ok && statusMatch) {
      passed++;
      vectors.push({
        cvId,
        formulaId: fx.formulaId,
      ...
      vectors.push({
        ...
        expected: CANONICAL_EXPECTED,
        tolerance: fx.tolerance,
        failureClass: statusMatch ? "IMPLEMENTATION_ERROR" : "STATUS_MISMATCH",
      });
    }
  }

  // Normative surface (independent of report shape)
  let surface = "FAIL";
  try {
  ...
  // Provisional green without schema (status must be set before schema validate)
  let green =
    passed === 44 &&
    failed === 0 &&
    blocked === 0 &&
  ...
  const report = {
    ...
    execution: {
      runtimeHead, // informational only; not authoritative
    },
    conformance: {
      suite: "0.1.4",
      range: "CV-01..CV-44",
    ...
    expectedStatusCheck: statusMatchFail ? "FAIL" : "PASS",
    traceability: "PASS",
    vectors,
    normativeSurface: surface,
    gate: {
  ...
  // Real JSON Schema validation (after status is set)
  ...
  if (fs.existsSync(schemaPath)) {
    try {
      ...
      const result = validateSchema(report, schema);
      schemaPass = result.valid;
      schemaErrors = result.errors || [];
    } catch (e) {
      schemaPass = false;
      schemaErrors = [String(e.message || e)];
    }
  } else {
    schemaErrors = ["schema file missing"];
  }
  report.reportSchema = schemaPass ? "PASS" : "FAIL";
  if (schemaErrors.length) report.schemaErrors = schemaErrors;

  // Final green includes schema
  green =
    green && schemaPass;
  ...
  // CLEAN WORKTREE DEFAULT: only write when --write
  if (WRITE) {
    fs.writeFileSync(
  ...
  console.log("AILEXSI CORE — PHASE 04 FINAL EVIDENCE CLOSURE");
  console.log("mode:", WRITE ? "WRITE (stable report)" : "VERIFY-ONLY (clean worktree)");
  console.log("runtimeHead (info):", runtimeHead);
  ...
  console.log("EXPECTED_STATUS_CHECK:", report.expectedStatusCheck);
  console.log("NORMATIVE SURFACE:", surface);
  console.log("REPORT SCHEMA:", report.reportSchema);
  if (schemaErrors.length) {
    console.log("SCHEMA ERRORS:", schemaErrors.slice(0, 5).join("; "));
  }
  ...
  if (!WRITE) {
    console.log("(no phase04-report.json written — worktree remains clean)");
  }
