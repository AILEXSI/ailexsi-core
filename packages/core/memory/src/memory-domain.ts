...
export class MemoryDomain {
  ...
  async create(cmd: CreateMemoryCommand): Promise<MemoryCell> {
    ...
    // Command-level idempotency: if key already exists, return projected state
    const existing = await this.store.getByIdempotencyKey(cmd.idempotencyKey);
    if (existing) {
      const existingId = existing.event.aggregateId;
      // Ensure projection has the cell (may need rebuild for this aggregate)
      if (!this.cells.has(existingId)) {
        const stream = await this.store.getByAggregate(existingId);
        for (const env of stream) this.applyEvent(env);
      }
      const cell = this.cells.get(existingId);
      if (cell) return cell;
    }
